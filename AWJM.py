...
def awjm(matrix, candidates, variables):
    ...
    for i in range(candidates):
        y1 = (
            ...
            + 0.00689 * matrix[i][2]
            - 0.00025 * matrix[i][3]
            + 0.00386 * matrix[i][1] * matrix[i][2]
        )
        y2 = (
            -0.93947 * matrix[i][1] * matrix[i][1]
            - 0.25711 * matrix[i][0] * matrix[i][1]
            - 0.00314 * matrix[i][0] * matrix[i][2]
            - 0.00249 * matrix[i][0] * matrix[i][3]
            + 0.00196 * matrix[i][1] * matrix[i][3]
            - 0.00002 * matrix[i][2] * matrix[i][3]
        )
        fx[i] = y1 + y2
...
if __name__ == "__main__":
    ...
    x_graph = []
    y_graph = []
    iterations = 500 #Number of iterations to execute the code for	
    for i in range(iterations):
        ...
        updatedIntervals = updateSamplingIntervals(
            matrix,
            newMatrix,
            upperbounds,
            lowerbounds,
